...
plot(alsfrs[which(alsfrs$ID == sample_0[1]),]$Delta, 
     alsfrs[which(alsfrs$ID == sample_0[1]),]$ALSFRS_R_Total, type = "b", col = "red", ylim = c(0,48),xlim = c(0,500), main = "Limb, dummy=0", xlab = "days", ylab = "ALSFRS-R")
...
plot(alsfrs[which(alsfrs$ID == sample_1[1]),]$Delta, 
     alsfrs[which(alsfrs$ID == sample_1[1]),]$ALSFRS_R_Total, type = "b", col = "forestgreen", ylim = c(0,48), xlim = c(0,500), main = "Bulbar, dummy = 1", xlab = "days", ylab = "ALSFRS-R")
...
########## subset -> training set
alsfrs_train = alsfrs[1:2012,] ### 202 patients
...
########## subset -> test set
alsfrs_test = alsfrs[2013:3001,] ### 93 patients
...
fit1 <- brm(data = alsfrs_train,
              family = gaussian,
              ALSFRS_R_Total ~ 1 + Delta + as.logical(dummy_bulbar) + 
                Delta*as.logical(dummy_bulbar) + (1 + Delta | ID),
