...
fit1 <- brm(data = alsfrs_train,
          family = gaussian,
          ALSFRS_R_Total ~ 1 + Delta*Onset_Site + (1 + Delta || ID),
          ...
          seed = 190831)
save(fit1, file = "brm_model_Onset.Rdata")
...
savePlot(filename = paste("plot_model2_beta2.","jpeg"), device = dev.cur())
...
savePlot(filename = paste("plot_model2_beta3.","jpeg"), device = dev.cur())

###### b_Interaction
chain <- data.out[,4]
x11()
layout(matrix(c(1,2,3,3),2,2,byrow=T))
plot(chain,type="l",main="Trace plot of beta4")
...
savePlot(filename = paste("plot_model2_beta4.","jpeg"), device = dev.cur())
...
### first 6 obs in training set, other 6 in test set
new_test = rbind(alsfrs_train[1:6,], alsfrs_train_patient_test_observation[1:6,])
data = predict(fit1, newdata = new_test)
x11()
plot(new_test$Delta, data[,1], ylim = c(0,48), type = "b", col = "red", pch = 20, xlab = "days", ylab = "ALSFRS")
